document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const res = await fetch('http://localhost:3000/api/posts/home');
    ...
    for (const post of posts) {
      ...
      let likeCount = 0;
      try {
        const likeRes = await fetch(`http://localhost:3000/api/likes/count/${post._id}`);
      ...
      let commentCount = 0;
      try {
        const commentRes = await fetch(`http://localhost:3000/api/comments/${post._id}`);
...
document.addEventListener("click", async (e) => {
  ...
  if (e.target.classList.contains("like-btn") || e.target.closest(".like-btn")) {
    ...
    try {
      // Gọi API toggle like
      await fetch(`http://localhost:3000/api/likes/toggle`, {
        method: "POST",
        headers: {
      ...
      const likeRes = await fetch(`http://localhost:3000/api/likes/count/${postId}`);
      const likeData = await likeRes.json();
